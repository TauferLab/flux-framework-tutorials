...
std::string writer_type = "decaf";

int main(int argc, const char** argv)
{
    if (argc != 8)
    {
        fprintf(stderr, "ERROR: ./generator json_conf py_path py_func file_path n_frames n_atoms delay_ms\n"); 
        return -1;
    }
    MPI_Init(NULL,NULL);
    ...
    if (app_rank == 0)
    {
        ...
        std::string file_path((char*)argv[4]);
        int n_frames = std::stoi(argv[5]);
        int n_atoms = std::stoi(argv[6]);
        int n_delay_ms = std::stoi(argv[7]);
